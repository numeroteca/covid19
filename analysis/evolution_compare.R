...
data_cases %>%
  ...
  geom_text_repel(data=filter( data_i_cases, date==max(data_i_cases$date), cases_per_100000 > 2 ), 
                  aes(date,cases_per_100000, 
                      label=paste(format(cases_per_100000, nsmall=1, big.mark="."), region)),
                  color="blue", 
                  nudge_x = 2, # adjust the starting y position of the text label
                  size=5,
                  hjust=0,
  ...
  labs(title = "Confirmed accumulated COVID-19 registed cases in Spain, Italy and France by 100.000 inhabitants by region",
       subtitle = paste0("By region (log scale). ",period),
