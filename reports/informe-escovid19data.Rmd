...
table_nice <- table_total %>% 
        ...
        select( province, ccaa, casos, casos_acum,  muertes, muertes_acum, PCR , PCR_diario) %>%
        arrange(casos_acum)
...
formattable(table_nice, list(
  area(col = c(casos, muertes,PCR)) ~ normalize_bar("pink", 0.2,na.rm = TRUE)
)), colnames = c('Provincia', 'CCAA', 'Casos acumulado (días desde)', 'Casos (fecha)', 'Muertes acumulado (días desde)', 'Muertes (fecha)','PCR diario (días desde)', 'PCR (fecha)'  ),
